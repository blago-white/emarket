...
from lunemarket.lunemarket_tests.tests_presets import BaseSingleUserTestCase, TEST_PHONE_DEFAULT_FIELDS
from lunemarket.lunemarket_tests import tests_utils

from ..filters import dashes_to_spaces
from ..models.models import Phones, Category
...
class PhonesModelTestCase(BaseSingleUserTestCase):
    ...
    def test_delete(self):
        ...
        self.assertEqual(Phones.objects.filter(category=self._test_category).count(), 1)
    ...
    def _add_second_test_product(self) -> Phones:
...
class CategoryModelTestCase(BaseSingleUserTestCase):
    ...
    # def setUp(self) -> None:
    #     super().setUp()
    #     self._test_parent_category = tests_utils.create_default_test_category()
    #     self._test_child_category = self._create_test_parent_category()
