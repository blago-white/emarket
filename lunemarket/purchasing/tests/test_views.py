...
from lunemarket.lunemarket_tests import tests_presets
...
class BuyProductViewTestCase(_BaseShoppingBasketViewTestCase):
    ...
    def _test_products_count_after_order(self, expected_products_count: int) -> None:
        self.assertEqual(Phones.objects.get(pk=self.test_product.id).products_count, expected_products_count)
